...
elseif q(1) == 1;
    ...
    if s(2) ==1 && s(3) ==0; %check if object is to the right of agent
        theta = s(5)+pi/6;
        d = d_max*rand(1); %new translation magnitude
    ...
    if s(2)==0 && s(3) ==0;
        if t<a;
        theta = s(5); %	 %new agent angle [note use of current angle, stored in sensor array as s(5)]
        d = d_max;
        end
        if t>=a;
