...
def rrt_path(obstacles, xy_start, xy_goal, params):
    ...
    while not nearGoal: # and iters < maxiters:
        ...
        if rnd < params.goal_prob:
            ...
            # xy = np.array([float(np.random.rand(1))*2*params.world_bounds_x[1]-params.world_bounds_x[1], float(np.random.rand(1))*2*params.world_bounds_x[1]-params.world_bounds_x[1]])
...
def ShortenPath(P, obstacles, smoothiters=10):
    ...
    while iters < smoothiters:
        s1 = random()*l[m-1] 
        s2 = random()*l[m-1]
        # s1 = float(np.random.rand(1))*l[m-1]
        # s2 = float(np.random.rand(1))*l[m-1]
        if s2 < s1:
            temps = s1
            s1 = s2
            s2 = temps
